...
const init = () => {
  getComponentConfiguration()
    .then((userInputs) => {
      ...
      // result: "Example"
      const componentNameUppercased = utils.upperFirstLetter(fullComponentName);

      // result: "user/local/desktop/project/component-name"
      const fullFolderPath = path.resolve(fullComponentName);
      // result: "user/local/desktop/project/component-name/component-name"
      const fullFilePath = path.resolve(
        `${fullComponentName}/${fullComponentName}`
      );
      const indexFullPath = path.resolve(`${fullComponentName}/index`);
      ...
      if (userInputs.createFilesList.includes("JSX")) {
        fs.writeFileSync(
          jsx,
          shortJsx(fullComponentName, componentNameUppercased)
        );
      }
